...
class R2ListResult:
    ...
    delimited_prefixes: list[str] = field(default_factory=_empty_str_list)
...
class R2Bucket(CloudflareService[Any]):
    ...
    async def list(
        ...
        *,
        ...
        objects = [object_info(item) for item in cast(list[Any], data.get("objects", []))]
        delimited_prefixes = [str(item) for item in data.get("delimitedPrefixes", [])]
...
def object_info(raw: Any, *, fallback_key: str | None = None) -> R2ObjectInfo:
    ...
    raw_http_metadata: Any = data.get("httpMetadata") or {}
    ...
    return R2ObjectInfo(
        ...
        size=get_r2_size(raw) if not isinstance(size, int) else int(size),
        etag=_optional_str(data.get("etag") or getattr(raw, "etag", None)),
        http_etag=_optional_str(data.get("httpEtag") or getattr(raw, "httpEtag", None)),
        ...
        custom_metadata=cast(dict[str, Any], custom_metadata)
        if isinstance(custom_metadata, dict)
        else None,
        storage_class=_optional_str(data.get("storageClass") or getattr(raw, "storageClass", None)),
        range=cast(dict[str, Any], range_value) if isinstance(range_value, dict) else None,
        checksums=cast(dict[str, Any], checksums) if isinstance(checksums, dict) else None,
    )
...
def _http_metadata_from_options(value: Any) -> R2HttpMetadata | None:
    ...
    return R2HttpMetadata(
        content_type=_optional_str(data.get("contentType")),
        content_language=_optional_str(data.get("contentLanguage")),
        content_disposition=_optional_str(data.get("contentDisposition")),
        content_encoding=_optional_str(data.get("contentEncoding")),
        cache_control=_optional_str(data.get("cacheControl")),
        cache_expiry=data.get("cacheExpiry"),
    )
