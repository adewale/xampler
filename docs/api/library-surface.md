...
5. Document operation capability: supported, caveated, demo-only, remote-only, unsupported/throws, or not covered.
6. Keep failure behavior predictable: absence is separate from auth/conflict/unsupported/provider failures.
7. Guard or lazy-load runtime-only and heavyweight optional dependencies.
8. Keep `.raw` available for platform escape hatches.
9. Keep demo transports explicit as `Demo*`, never hidden as if real products ran locally.
