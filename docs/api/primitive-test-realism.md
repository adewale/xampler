...
| Workers KV | `examples/storage-data/kv-namespace` + `xampler.kv` | 4 | `uv run python scripts/verify_examples.py examples/storage-data/kv-namespace` | Starts Worker locally, writes/reads text, writes/reads JSON, lists keys, deletes a key, and verifies a missing-key 404 through the shared wrapper. | Verify TTL/expiration behavior and deployed KV namespace. |
...
| D1 database | `examples/storage-data/d1-database` + `xampler.d1` | 4 | `uv run python scripts/verify_examples.py examples/storage-data/d1-database` | Initializes local D1 with `db_init.sql`, creates an index, runs `PRAGMA optimize`, starts the Worker, verifies a seeded row, verifies bound query, and checks an `EXPLAIN QUERY PLAN` route for index use through the shared wrapper. | Add write/query mutation route, retry guidance, and remote/deployed D1 verification. |
